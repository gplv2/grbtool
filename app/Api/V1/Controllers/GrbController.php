...
class GrbController extends Controller
{
   ...
   public function index(Request $request)
   {
      ...
      if(!strlen($output)) {
         ...
         $sql  = "SELECT " . sprintf(pg_escape_string($fields), $tags) . ", ST_AsGeoJSON(ST_Transform(" . pg_escape_string($geomfield) . ",$srid::int),15,4) AS geojson FROM " . pg_escape_string($geotable);
   ...
   public function present(Request $request)
   {
	...
	$msg=array('error' => 'No GRB ref in request');
	$response = new Response();
	$response->header('charset', 'utf-8');
	return response()->json($msg);
      }
      ...
      if(!strlen($output)) {
         ...
         // Build RAW SQL SELECT statement and return the geometry as a GeoJSON element in EPSG: 4326
         $sql  = "SELECT " . sprintf(pg_escape_string($fields), $tags) . ", ST_AsGeoJSON(ST_Transform(" . pg_escape_string($geomfield) . ",$srid::int),15,4) AS geojson FROM " . pg_escape_string($geotable);
         $sql .= sprintf(" WHERE " . pg_escape_string("way") . " AND source:geometry:ref='". $ref . "'  && ST_SetSRID('BOX3D(%s %s, %s %s)'::box3d, %s)", $bbox_west, $bbox_south, $bbox_east, $bbox_north, $srid);
         // $sql  = "SELECT " . sprintf(pg_escape_string($fields)) . ", ST_AsGeoJSON(ST_Transform(" . pg_escape_string("way") . ",$srid),15,4) AS geojson FROM " . pg_escape_string($geotable);
